...
function RoutinePreview({ block }: BlockPreviewProps) {
  ...
  return (
    <div className="p-3 flex flex-col gap-1">
      {content.slots.slice(0, 8).map((slot, i) => (
        <div key={slot.id ?? i} className="flex items-center gap-3 text-xs py-1" style={{ borderBottom: '1px solid var(--planner-border)' }}>
...
function RoutinePrint({ block }: { block: import('@/features/storage/types').PlannerBlock }) {
  ...
  return (
    <div style={{ padding: '8pt' }}>
      {content.slots.map((slot, i) => (
        <div key={slot.id ?? i} style={{ display: 'flex', gap: '12pt', borderBottom: '1px solid #eee', padding: '4pt 0', fontSize: '10pt' }}>
